...
# draws the contours of the cube
def draw_cube_lines(img, pts2d):
    # draw pillars
...
def draw_distance_cube(img, imgpts, rvec, tvec, cube_3d, K, dist, size):
    pts2d = np.int32(imgpts).reshape(-1, 2)
    top2d = pts2d[4:8]

    img = draw_cube_lines(img, pts2d)
    ...
    # convert distance to meters (1 printed square side is 0.025 meters)
    dist_units = float(np.linalg.norm(center_cam))
    dist_m = dist_units * size
...
def draw(img, corners, rvecs, tvecs, K, dist, size, no_cube=False):
    cube = np.float32([
    ...
    imgpts, _ = cv.projectPoints(axes, rvecs, tvecs, K, dist)
    img = draw_axes(img, corners, imgpts)

    if no_cube: return img

    imgpts, _ = cv.projectPoints(cube, rvecs, tvecs, K, dist)
    img = draw_distance_cube(img, imgpts, rvecs, tvecs, cube, K, dist, size)
